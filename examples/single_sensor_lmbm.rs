//! Single-sensor LMBM filter example
//!
//! This example demonstrates the basic usage of the LMBM (Labelled Multi-Bernoulli Mixture)
//! filter for multi-object tracking with a single sensor. The LMBM filter maintains multiple
//! hypotheses for improved tracking accuracy.
//!
//! This matches the MATLAB runFilters.m script with useLmbFilter = false.
...
fn main() {
    println!("=== Single-Sensor LMBM Filter Example ===\n");
    ...
    // Run LMBM filter
    println!("\nRunning LMBM filter...");
    let state_estimates = run_lmbm_filter(&model, &ground_truth_output.measurements);
