//! Centralized numpy ↔ nalgebra conversion functions
//!
//! All conversion functions are defined here to avoid duplication across modules.
...
pub fn numpy_to_dvector(arr: PyReadonlyArray1<'_, f64>) -> DVector<f64> {
    DVector::from_vec(arr.as_slice().unwrap().to_vec())
}

/// Convert numpy 2D array to nalgebra DMatrix (row-major)
pub fn numpy_to_dmatrix(arr: PyReadonlyArray2<'_, f64>) -> DMatrix<f64> {
    let shape = arr.shape();
    let rows = shape[0];
    let cols = shape[1];
...
pub fn dvector_to_numpy<'py>(py: Python<'py>, v: &DVector<f64>) -> Bound<'py, PyArray1<f64>> {
    PyArray1::from_slice(py, v.as_slice())
}

/// Convert nalgebra DMatrix to numpy 2D array (row-major)
pub fn dmatrix_to_numpy<'py>(py: Python<'py>, m: &DMatrix<f64>) -> Bound<'py, PyArray2<f64>> {
    let rows = m.nrows();
    let cols = m.ncols();
...
pub fn numpy_nested_to_measurements(
