...
use crate::multisensor_lmbm::association::generate_multisensor_lmbm_likelihood_only;
use crate::multisensor_lmbm::gibbs::multisensor_lmbm_gibbs_sampling;
use crate::multisensor_lmbm::hypothesis::determine_multisensor_posterior_hypothesis_parameters_deferred;
    ...
    for t in 0..simulation_length {
        ...
        for i in 0..hypotheses.len() {
            ...
            if measurements_are_available {
                // Phase C: Generate likelihood matrix ONLY (no posterior params yet)
                let (l, dimensions) = generate_multisensor_lmbm_likelihood_only(
                    &prior_hypothesis,
                    &measurements_t,
                    model,
                    number_of_sensors,
                ...
                // Phase C: Determine posterior params ONLY for sampled indices (~1000 vs 10.7M)
                let new_hypotheses = determine_multisensor_posterior_hypothesis_parameters_deferred(
                    &a,
                    &l,
                    &dimensions,
                    &prior_hypothesis,
