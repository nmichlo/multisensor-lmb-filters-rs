...
use multisensor_lmb_filters_rs::lmb::{
    association::generate_lmb_association_matrices, cardinality::elementary_symmetric_function,
    filter::run_lmb_filter, prediction::lmb_prediction_step,
};
...
fn create_test_objects(
    ...
    n_objects: usize,
    n_gm: usize,
) -> Vec<multisensor_lmb_filters_rs::common::types::Object> {
...
fn bench_association_matrices(c: &mut Criterion) {
    ...
    for (n_obj, n_meas) in [(5, 5), (10, 10), (20, 20), (50, 50)] {
        ...
        group.bench_with_input(
            ...
            |b, (obj, meas, mdl)| b.iter(|| generate_lmb_association_matrices(obj, meas, mdl)),
        );
    }
...
fn create_association_matrices(n_obj: usize, n_meas: usize) -> AssociationMatrices {
    ...
    let psi = DMatrix::from_fn(n_obj, n_meas, |i, j| {
...
fn bench_lbp(c: &mut Criterion) {
    ...
    for (n_obj, n_meas) in [(5, 5), (10, 10), (20, 20), (50, 50)] {
        ...
        group.bench_with_input(
            ...
            &matrices,
            |b, m| b.iter(|| loopy_belief_propagation(m, 1e-6, 100)),
        );
    }
...
fn bench_esf(c: &mut Criterion) {
    ...
    for n in [5, 10, 20, 50, 100] {
        ...
        group.bench_with_input(
            ...
            &z,
            |b, z| b.iter(|| elementary_symmetric_function(z)),
        );
    }
...
fn bench_prediction(c: &mut Criterion) {
    ...
    for n_obj in [5, 10, 20, 50] {
        ...
        group.bench_with_input(
            ...
            &(&objects, &model),
            |b, (obj, mdl)| b.iter(|| lmb_prediction_step(obj.to_vec(), mdl, 1)),
        );
    }
...
fn bench_full_filter(c: &mut Criterion) {
    ...
    for sim_length in [10, 25, 50] {
        ...
        let measurements: Vec<Vec<DVector<f64>>> = ground_truth
            .measurements
            .into_iter()
            .take(sim_length)
            .collect();
