//! Detection probability sensitivity trial tests - Validate exact numerical equivalence with MATLAB
//!
//! This test suite validates that the Rust implementation produces identical results
//! to the MATLAB implementation for detection probability sensitivity trials. It uses
//! deterministic fixtures generated from MATLAB with SimpleRng to enable exact numerical comparison.
//!
//! Based on MATLAB singleSensorDetectionProbabilityTrial.m.
...
/// Complete fixture data for detection probability sensitivity trial
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Test helper: validate a detection probability sensitivity fixture
fn validate_detection_fixture(filename: &str) {
    println!("\n=== Testing {} ===", filename);
    ...
    println!("  Clutter rate: {}", fixture.clutter_rate);
    println!(
        "  Simulation lengths: Gibbs={}, Other={}",
        fixture.simulation_length_gibbs, fixture.simulation_length_other
    );

    const TOLERANCE: f64 = 1e-8; // Relaxed for averaging across timesteps
    ...
    for variant in &fixture.filter_variants {
        ...
        // Run trial for each detection probability
        let mut rust_e_ospa = Vec::with_capacity(fixture.detection_probabilities.len());
        let mut rust_h_ospa = Vec::with_capacity(fixture.detection_probabilities.len());

        for (i, &detection_prob) in fixture.detection_probabilities.iter().enumerate() {
            let (mean_e, mean_h) = if is_lmbm {
                run_detection_trial_lmbm(
                    fixture.seed,
                    method,
                    fixture.clutter_rate,
                    detection_prob,
                    simulation_length,
                )
            } else {
            ...
            println!(
                "    Detection prob {:.3}: MATLAB E-OSPA={:.6}, Rust E-OSPA={:.6}, diff={:.2e}",
                detection_prob,
                variant.e_ospa[i],
                mean_e,
        ...
        for (i, (rust_e, matlab_e)) in rust_e_ospa.iter().zip(variant.e_ospa.iter()).enumerate() {
            ...
            assert!(
                diff < TOLERANCE,
                "E-OSPA mismatch at detection prob {} for {}: Rust={}, MATLAB={}, diff={}",
                fixture.detection_probabilities[i],
                variant.name,
                rust_e,
                matlab_e,
                diff
        ...
        for (i, (rust_h, matlab_h)) in rust_h_ospa.iter().zip(variant.h_ospa.iter()).enumerate() {
            ...
            assert!(
                diff < TOLERANCE,
                "H-OSPA mismatch at detection prob {} for {}: Rust={}, MATLAB={}, diff={}",
                fixture.detection_probabilities[i],
                variant.name,
                rust_h,
                matlab_h,
...
#[test]
fn test_detection_probability_sensitivity_quick() {
    validate_detection_fixture("single_detection_trial_42_quick.json");
}
...
#[test]
fn test_detection_trial_determinism() {
    // Verify that running the same seed twice produces identical results
    let seed = 42;
    let method = DataAssociationMethod::LBP;
    let clutter_rate = 5.0;
    let detection_prob = 0.95;
    let sim_length = 100;

    let (e1, h1) = run_detection_trial(seed, method, clutter_rate, detection_prob, sim_length);
    let (e2, h2) = run_detection_trial(seed, method, clutter_rate, detection_prob, sim_length);
